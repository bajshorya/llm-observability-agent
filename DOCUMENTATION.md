# Technical Documentation — Phase 0

Complete reference for everything built so far: every file, what its code does,
how it works, and why it was written that way.

**Scope:** Phase 0 (foundations). 14 TypeScript files, ~1,200 lines, plus config.
No LLM code exists yet — that is Phase 2.
...
A concrete incident, end to end. Phase 0 has built steps 1–3.
...
### Phase 1 — statistical detection (no LLM)

1. **Rollup worker** — raw logs → `metrics_rollup`, computing per-minute counts and
   p50/p95/p99. Idempotent via the unique index.
2. **Error-rate detector** — flag when error count exceeds `mean + k·stddev` of a
   trailing baseline window.
3. **Latency detector** — flag when p95/p99 jumps beyond a threshold ratio versus
   baseline.
4. **New-signature detector** — flag signatures absent from the baseline window.
   This is where the §11.1 groundwork pays off.
5. **Write anomalies**, then confirm each scenario triggers its own detector and
   only its own.

At the end of Phase 1 there is a **working anomaly detector with zero LLM
involvement** — the honest baseline the entire two-tier argument rests on.

### Later phases

- **Phase 2** — LLM classifier behind a provider interface (Gemini / NVIDIA /
  OpenRouter / Ollama / stub), Zod-validated with a parse-repair loop, plus token
  and latency logging.
